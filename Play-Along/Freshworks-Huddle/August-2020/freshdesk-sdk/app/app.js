...
function createTicket() {
  ...
  client.request
    .invoke("createTicket", properties)
    .then(function (data) {
      console.info("Ticket create successfull");
      console.info(JSON.stringify(data));
      showNotification(
        "success",
        `Ticket created successfully! Ticket ID: ${data.response.data.id}`
      );
    })
    .catch(function (error) {
...
function updateTicket() {
  ...
  client.request
    .invoke("updateTicket", properties)
    .then(function (data) {
...
function getTicket() {
  const ticketId = 12;
  client.request
    .invoke("getTicket", { ticketId })
    .then(function (data) {
      console.info("Ticket fetch successfull");
      console.info(JSON.stringify(data));
      showNotification(
        "success",
        `Ticket got successfully! Ticket ID: ${ticketId} and Priority: ${data.response.data.priority}`
      );
    })
    .catch(function (error) {
...
function searchContact() {
  client.request
    .invoke("searchContact", {
      query: document.getElementById("contactEmail").value,
    })
    .then(function (data) {
      ...
      showNotification(
        "success",
        `Contact search successful! Search results: ${data.response.data.total}`
      );
    })
    .catch(function (error) {
