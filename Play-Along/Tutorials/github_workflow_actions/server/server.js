...
exports = {
  inviteUser: function (args) {
    ...
    $request.put(url, options).then(
      (data) => {
        renderData(null, { success: true, data: data.response });
      },
      (error) => {
        console.log(
          "Failed to invite the user to the GitHub organization.",
          error.response.message
        );
        renderData(null, {
          success: false,
          error: "Failed to invite the user to the GitHub organization",
        });
      }
    );
  },
  ...
  deleteUser: function (args) {
    ...
    $request.delete(url, options).then(
      (data) => {
        renderData(null, { success: true, data: data });
      },
      (error) => {
        console.log(
          "Failed to delete the user from the GitHub organization.",
          error.response.message
        );
        renderData(null, {
          success: false,
          error: "Failed to delete the user from the GitHub organization",
        });
      }
    );
  },
