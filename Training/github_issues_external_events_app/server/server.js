...
exports = {
    ...
    generateTargetUrl().then(function (targetUrl) {
      $request.post(`https://api.github.com/repos/${args.iparams.github_username}/${args.iparams.github_repo_name}/hooks`, {
        ...
        $db.set('githubWebhookId', { url: data.response.url }).then(function () {
          ...
          console.log('Error: Failed to store the webhook URL in the db');
          console.log(error);
          renderData({ message: 'The webhook registration failed' });
        });
      }, error => {
        console.log('Error: Failed to register the webhook for GitHub repo');
        console.log(error);
        renderData({ message: 'The webhook registration failed' });
      })
    })
  ...
  onWebhookCallbackHandler: function (payload) {
    ...
    if (payloadData.action === 'opened') {
      $request.post(payload.domain + "/api/v2/tickets",
        {
          headers: {
