...
function storeInteraction(ticketData) {
  client.db.set(`ticket: ${ticketData.id}`, { "interaction_noted": true }, { setIf: "not_exist" }).then(() => {
    updateInteraction(ticketData.requester_id);
    getLoggedUserData(ticketData.requester_id);
  }, error => {
    notify('warning', 'Interaction has already been stored for this ticket');
    console.error(error, 'Interaction has already been stored for this ticket');
  })
}
/**
...
function getAssociatedCustomers(agentId, requesterId) {
  client.db.get(`agent_id: ${agentId}`).then(data => {
...
function appendCustomerToAgent(agentId, requesterId) {
  client.db.update(`agent_id: ${agentId}`, "append", { associated_customers: [requesterId] }).then(() => {
