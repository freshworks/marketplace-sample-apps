...
exports = {
  ...
  onTicketUpdateHandler: function (args) {
    ...
    if (ticket.status == TICKET_STATUS.closed) {
      console.log(
        "Ticket has been closed. Updating the respective restaurant record based on the ticket ID filter"
      );
      ...
      appointmentRecords.then(
        (appointment) => {
          console.log("Appointments with ticket ID ", ticket_id, appointment);
          restaurants
            .get(appointment.records[0].data.restaurant_id)
            .then((restaurant) => {
              console.log(
                "Restaurant with matching ticket id ",
                restaurant.record.data
              );
              let updatedObj = restaurant.record.data;
              updatedObj.status = TICKET_STATUS.pending;
              restaurants.update(restaurant.record.display_id, updatedObj).then(
                (updatedRecord) => {
