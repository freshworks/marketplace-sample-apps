  ...
  app.initialized().then(function (client) {
    ...
    Promise.all([getEntity("restaurants"), getEntity("appointments")])
      ...
      .catch(function (error) {
        notify(
          "danger",
          "Something went wrong while obtaining reference to `restaurants` & `appointments` entity. Check the console for details."
        );
        console.error(error);
      });
...
function showNewRestaurantModal() {
  client.interface
    ...
    .catch(function (error) {
      notify(
        "danger",
        "Something went wrong while opening the restaurant modal"
      );
      console.error(error);
    });
}
  ...
  return new Promise(function (resolve, reject) {
    ...
    entityRef
      ...
      .catch(function (error) {
        notify(
          "danger",
          `Error occurred while obtaining reference to '${entityName}' entity`
        );
        console.error(error);
        reject(entityRef);
      });
  ...
  restaurant.getAll().then(
    function (data) {
      document.getElementById("restaurant_list").innerHTML =
        generateRestaurantList(data.records);
    },
    function (error) {
  ...
  if (typeof client != "undefined") {
    client.iparams
      ...
      .then(function (iparam) {
        appointment
          ...
          .catch(function (error) {
            notify(
              "danger",
              "Something went wrong while loading all the appointments"
            );
            console.error(error);
          });
      })
      .catch(function (error) {
        notify(
          "danger",
          "Something went wrong while retrieving the installation parameters"
        );
        console.error(error);
      });
  } else {
    ...
    .map(function (restaurant) {
      return `<li class="list-group-item">
            <div class="row">
              <div class="col-sm-3">
                <img src='${
                  restaurant.data.photo_url
                    ? restaurant.data.photo_url
                    : "https://i.imgur.com/ZyONF7N.png"
              ...
              <div class="col-sm-6 ml-2" style='min-width: 350px;' >
                ...
                <a href="#">
                    <fw-label value="${
                      CATALOG_STATUS[restaurant.data.status].status
                    }" color="${CATALOG_STATUS[restaurant.data.status].color}"></fw-label>
                    <fw-button size="mini" style="display:${
                      restaurant.data.status == "1" ? "inline" : "none"
                    }" color="secondary" onclick='deleteRestaurant("${restaurant.display_id}","${escape(restaurant.data.name)}")'> Delete </fw-button>
                </a>
              </div>
              <div class="col-sm-3">
...
function deleteRestaurant(id, name) {
  client.interface
    .trigger("showConfirm", {
      ...
      message:
        "Note: You also cannot delete restaurants if they already have appointments",
      saveLabel: "Delete",
    })
    .then(function (action) {
      if (action.message == "Delete") {
        ...
        notify(
          "success",
          `Restaurant - ${unescape(name)} deleted successfully`
        );
        updateView();
      }
    })
