 ...
 */
function createSubscriptionForHubspotContactCreation(
  appId,
  hubspotUrl,
  hubspotKey
) {
  ...
  $request.post(url, reqData).then(function (data, err) {
    if (err) {
      console.error(
        `Subscription for Hubspot event ${CONTACT_CREATION} failed.`,
        JSON.stringify(err)
      );
    }
  });
 ...
 */
function createFreshdeskContact(
  freshdeskDomain,
  hubspotName,
  hubspotEmail,
  hubspotJobtitle
  ...
  $request.post(url, reqData).then(function (data, err) {
    if (err) {
      console.error(
        "Contact creation failed in Freshdesk.",
        JSON.stringify(err)
      );
    }
  });
...
function deleteSubscription(appId, hubspotUrl, hubspotKey, subscriptionId) {
  ...
  $request.delete(url, {}).then(function (data, err) {
    if (err) {
      console.error(
        `Subscription ${subscriptionId} deletion failed.`,
        JSON.stringify(err)
      );
    }
...
exports = {
  ...
  onAppInstallHandler: function (args) {
    generateTargetUrl().then(
      function (targetUrl) {
        ...
        $request.put(url, reqData).then(
          function (data) {
            console.log(JSON.stringify(data));
            renderData();
            createSubscriptionForHubspotContactCreation(
              args.iparams.hubspot_app_id,
              args.iparams.hubspot_url,
              args.iparams.hubspot_dev_key
            );
          },
          function (err) {
            console.error(
              "Webhook registration with Hubspot failed.",
              JSON.stringify(err)
            );
            renderData({
  ...
  onContactCreateCallback: function (args) {
    ...
    $request.post(url, reqData).then(function (data, err) {
      if (err) {
        console.error(
          "Contact creation in HubSpot failed.",
          JSON.stringify(err)
        );
      }
    });
  ...
  onHubSpotCallbackHandler: function (args) {
    if (args.data[0].subscriptionType === CONTACT_CREATION) {
      ...
      $request.get(url, reqData).then(
        ...
        function (err) {
          console.error(
            "Contact fetching failed from Hubspot.",
            JSON.stringify(err)
          );
        }
      );
  ...
  onHubSpotUninstallHandler: function (args) {
    ...
    $request.get(url, {}).then(
      ...
      function (err) {
        console.log(
          "Error fetching list of subscriptions",
          JSON.stringify(err)
        );
        renderData({
