...
function validateFields() {
  ...
  if (
    (new Date(scheduleData.scheduleAt) - new Date(Date.now())) / 1000 / 60 <
    10
  ) {
    q("#scheduleAt-error").innerHTML =
      "Schedule time must be at least 10 minutes in future";
    isValid = false;
  }
...
function getListOfSchedules(userId, callback) {
  client.db.get(`schedules_${userId}`).then(
    function (data) {
      callback(data);
    },
    function (err) {
...
function addScheduleToList(schedule, callback) {
  getListOfSchedules(loggedInUserId, function (data) {
    ...
    client.db.set(`schedules_${loggedInUserId}`, data).then(
      function () {
        callback();
      },
      function (err) {
        ...
        sendNotification(
          DANGER_NOTIFICATION,
          `Error storing list of schedules`
        );
      }
    );
  });
