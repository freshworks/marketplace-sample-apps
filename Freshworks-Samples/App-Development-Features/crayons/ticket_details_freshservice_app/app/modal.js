document.addEventListener("DOMContentLoaded", function () {
  ...
  app.initialized().then(
    ...
    function () {
      notify(
        "info",
        "Unable to Display Ticket Details, kindly refresh the page "
      );
    }
  );
  ...
  function displayInHtml(data) {
    if (isEmpty(data)) {
      ...
      document
        .getElementById("renderOutput")
        .insertAdjacentHTML(
          "beforeend",
          "<p> No Content / Unable to Show content </p>"
        );
      ...
      document
        .getElementById("renderOutput")
        .insertAdjacentHTML("beforeend", '<ul id="items"> </ul>');

      for (const [dataKey, dataValue] of Object.entries(data)) {
        let html = "";
        html += `<ul id="items"> <li class="clearfix"><label class="info">${dataKey}</label><label class="value">${convertToString(
          dataValue
        )}</label></li></ul>`;
  ...
  function convertToString(data) {
    ...
    if (typeof dataValue == "object") {
      objectToString +=
        "<div class='nested'>" + JSON.stringify(dataValue) + "</div>";
