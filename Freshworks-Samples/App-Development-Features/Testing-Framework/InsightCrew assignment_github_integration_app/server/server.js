...
exports = {
  ...
  onWebhookCallbackHandler: function (payload) {
    ...
    $request
      .get(`https://${payload.domain}/api/v2/tickets`, {
        headers: {
      ...
      .then(
        (data) => {
          ...
          if (payloadData.action == "closed") {
            $request
              .put(`https://${payload.domain}/api/v2/tickets/${ticket[0].id}`, {
                headers: {
                  Authorization: "<%= encode(iparam.freshdesk_api_key) %>",
                },
                json: {
              ...
              .then(
                () => {
                  console.info(
                    "Successfully updated status of ticket in Freshdesk"
                  );
                },
                (error) => {
                  console.error(
                    "Error in updating status of ticket in Freshdesk"
                  );
                  renderData({
                    message: error,
                  });
  ...
  onTicketCreateHandler: function (args) {
    const payloadData =
      typeof args.data === "string" ? JSON.parse(args.data) : args.data;
    let label = [];
    label.push(payloadData.ticket.type);
    $request
      .post(
        `https://api.github.com/repos/${args.iparams.github_username}/${args.iparams.github_repo_name}/issues`,
        {
          headers: {
            Authorization: "token <%= access_token %>",
            "User-Agent": "Awesome-Octocat-App",
      ...
      .then(
        (data) => {
          $request
            .put(
              `https://${args.domain}/api/v2/tickets/${payloadData.ticket.id}`,
              {
                headers: {
                  Authorization: "<%= encode(iparam.freshdesk_api_key) %>",
                },
            ...
            .then(
              () => {
                console.info(
                  "Successfully updated custom fields of ticket in Freshdesk"
                );
              },
              (error) => {
                console.error(
                  "Error in updating custom fields of ticket in Freshdesk"
                );
              }
            );
        },
  ...
  onTicketUpdateHandler: function (args) {
    ...
    $request
      .get(
        `https://${args.domain}/api/v2/tickets/${payloadData.ticket.custom_fields[freshdesk_ticket_id_field]}`,
        {
          headers: {
            Authorization: "<%= encode(iparam.freshdesk_api_key) %>",
          },
      ...
      .then(
        (data) => {
          ...
          $request
            .patch(
              `https://api.github.com/repos/${args.iparams.github_username}/${args.iparams.github_repo_name}/issues/${ticketData.custom_fields.cf_github_ticket_number}`,
              {
                headers: {
                  Authorization: "token <%= access_token %>",
                  "User-Agent": "Awesome-Octocat-App",
  ...
  onConversationCreateHandler: function (args) {
    ...
    $request
      .get(
        `https://${args.domain}/api/v2/tickets/${payloadData.conversation.ticket_id}`,
        {
          headers: {
            Authorization: "<%= encode(iparam.freshdesk_api_key) %>",
            "User-Agent": "Awesome-Octocat-App",
      ...
      .then(
        (data) => {
          ...
          $request
            .post(
              `https://api.github.com/repos/${args.iparams.github_username}/${args.iparams.github_repo_name}/issues/${ticketData.custom_fields.cf_github_ticket_number}/comments`,
              {
                headers: {
                  Authorization: "token <%= access_token %>",
                  "User-Agent": "Awesome-Octocat-App",
        ...
        (error) => {
          console.error(
            "Error in finding the github ticket id for adding comment"
          );
          renderData({
            message: error,
          });
