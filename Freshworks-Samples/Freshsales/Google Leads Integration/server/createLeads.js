...
const constructFreshsalesPayload = (googleLeadsPayload, iparams) => {
  ...
  for (let fields in googleLeadsPayload["user_column_data"]) {
    if (
      googleLeadsPayload["user_column_data"][fields]["column_id"] ===
      "FULL_NAME"
    ) {
      freshsalesCreateLeadPayload = handleName(
        googleLeadsPayload["user_column_data"][fields]["string_value"],
        freshsalesCreateLeadPayload
      );
    } else if (
      googleLeadsPayload["user_column_data"][fields]["column_id"] in leadMapping
    ) {
      freshsalesCreateLeadPayload["lead"][
        leadMapping[googleLeadsPayload["user_column_data"][fields]["column_id"]]
      ] = googleLeadsPayload["user_column_data"][fields]["string_value"];
    }
  }
...
function createLeadinFreshsales(freshsalesPayload, iparams) {
  ...
  $request.post(createLeadsUrl, options).then(
    function (data) {
