...
function getRequestOptions(email) {
  return buildOptions(
    searchContactUrl + "?query=\"email:'" + email + "'\"",
    "GET",
    headers,
    null
  );
...
async function fetchContactIdFromFreshdesk(freshsalesContact) {
  try {
    const data = await utils.makeGetRequest(
      getRequestOptions(freshsalesContact.email)
    );
    ...
    try {
      await contacts.setContactMapping(
        freshsalesContact.id,
        data.response.results[0].id
      );
      return data.response.results[0].id;
    } catch (error) {
      console.log(
        "failed to store the contact mapping of contact id:",
        freshsalesContact.id,
        " with freshdesk contact id:",
        data.response.results[0].id
      );
      console.log(error);
      throw error;
    ...
    console.log(
      "got error in making get request to freshdesk to fetch contact info"
    );
    console.log(error);
    throw error;
  }
...
async function getFreshdeskContactId(payload) {
  try {
    ...
    return data
      ? data
      : await fetchContactIdFromFreshdesk(payload.data.contact);
  } catch (error) {
...
function addName(firstName, newFirstName, lastName, newLastName) {
  ...
  return {
    name:
      ((newFirstName && newFirstName[1]) || firstName) +
      " " +
      ((newLastName && newLastName[1]) || lastName),
  };
}
      ...
      if (!utils.isEmpty(changes)) {
        try {
          await utils.makePutRequest(
            putRequestOptions(freshdeskContactId, changes)
          );
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.log(
            "unable to remove the contact mapping after deletion of contact with error"
          );
          console.log(error);
        }
      } catch (error) {
