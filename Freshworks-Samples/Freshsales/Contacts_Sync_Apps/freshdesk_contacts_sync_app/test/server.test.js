...
describe('Product events tests', function () {
  ...
  describe('onContactCreate event', function () {
    ...
    it('checks when a contact already exists in the db', async function () {
      ...
      expect(stubbedDb.calledWith('contact:' + onContactCreateArgs.data.contact.id)).to.eql(true);
    });
    ...
    it('checks when a contact 1. does not exist in the db and 2. fetches from API and 3. store mapping in db successfully', async function () {
      ...
      stubbedDb.resolves({});
      stubbedGetRequest.resolves({ response: '{"results":[{"id":' + FRESHDESK_CONTACT_ID + '}]}' });


      await this.invoke('onContactCreate', onContactCreateArgs);

      expect(stubbedGetRequest.calledWith(`https://ravirajsubramanian.freshdesk.com/api/v2/search/contacts?query="email:'${onContactCreateArgs.data.contact.email}'"`, { headers: { Authorization: 'Basic <%= encode(iparam.freshdesk_api_key)%>' } })).to.eql(true);
      expect(stubbedUpdateDb.calledWith('contact:' + onContactCreateArgs.data.contact.id, 'set', { 'freshdesk_contact_id': FRESHDESK_CONTACT_ID })).to.eql(true);
    });
    ...
    it('checks when a contact does not exist and creates new contact in FD successfully', async function () {
      ...
      expect(stubbedPostRequest.calledWith('https://ravirajsubramanian.freshdesk.com/api/v2/contacts/', {
        headers: { Authorization: 'Basic <%= encode(iparam.freshdesk_api_key)%>' },
        json: {
          ...
          name: onContactCreateArgs.data.contact.first_name + ' ' + onContactCreateArgs.data.contact.last_name,
          twitter_id: onContactCreateArgs.data.contact.twitter.split('https://twitter.com/')[1]
        }
      })).to.eql(true);
    });
  ...
  describe('onContactUpdate event', function () {
    ...
    it('checks when a contact already exists in the db', async function () {
      ...
      expect(stubbedPutRequest.calledWith(`https://ravirajsubramanian.freshdesk.com/api/v2/contacts/${FRESHDESK_CONTACT_ID}`, {
        headers: { Authorization: 'Basic <%= encode(iparam.freshdesk_api_key)%>' },
        json: {
    ...
    it('checks no changes in the attributes', async function () {
      ...
      await this.invoke('onContactUpdate', { data: { contact: { id: 24, changes: {} } } });

      expect(stubbedDb.calledWith('contact:' + onContactUpdateArgs.data.contact.id)).to.eql(true);
    });
  ...
  describe('onContactDelete event', function () {
    ...
    it('checks when a contact already exists in the db', async function () {
      ...
      expect(stubbedDb.calledWith('contact:' + onContactUpdateArgs.data.contact.id)).to.eql(true);
      expect(stubbedDeleteRequest.calledWith(`https://ravirajsubramanian.freshdesk.com/api/v2/contacts/${FRESHDESK_CONTACT_ID}`, { headers: { Authorization: 'Basic <%= encode(iparam.freshdesk_api_key)%>' } })).to.eql(true);
      expect(stubbedDbDelete.calledWith(`contact:${onContactDeleteArgs.data.contact.id}`)).to.eql(true);
    });
