  ...
  for (let data in payload.fields) {
    if (data in leadmapping) {
      if (data === "Name & organization") {
        payloadData["company"][leadmapping[data]] =
          payload.fields[data].split(",")[1];
      } else {
...
function checkTime(createdTime) {
  if (
    moment(maxTime, "yyyy-MM-DDTHH:mm:ss.000").isBefore(
      moment(createdTime, "yyyy-MM-DDTHH:mm:ss.000")
    )
  ) {
    moment(maxTime, "yyyy-MM-DDTHH:mm:ss.000").isBefore(
      moment(createdTime, "yyyy-MM-DDTHH:mm:ss.000")
