...
function searchContact(email) {
  return new Promise(function (resolve) {
    client.iparams.get("subdomain").then(function (iparam) {
      ...
      client.request
        .get(BASE_URL + "/api/search?q=" + email + "&include=contact", {
          headers: {
...
function fetchContactDetails(contactId) {
  return new Promise(function (resolve) {
    client.iparams.get("subdomain").then(function (iparam) {
      var BASE_URL = `https://${iparam.subdomain}.freshsales.io`;
      client.request
        .get(BASE_URL + "/api/contacts/" + contactId, {
          headers: {
            Authorization: AUTHORIZATION_TEMPLATE,
          },
        })
...
document.addEventListener("DOMContentLoaded", function (event) {
  async.waterfall([
    ...
    function (callback) {
      ...
      getTicketContact().then(function (contactInformation) {
    ...
    function (contactInformation, callback) {
      // Search CRM
      searchContact(contactInformation.requester.email).then(function (
        searchResult
      ) {
    ...
    function (searchResult, callback) {
      if (searchResult.length > 0) {
        ...
        fetchContactDetails(searchResult[0].id).then(function (
        ...
        document
          .getElementById("contact-info")
          .appendChild(
            '<div class="fw-content-list"><div class="muted">Contact not found</div></div>'
          );
      }
    },
    ...
    function (crmContactInformation) {
      for (var contactKey in CONTACT_INFO_MAPPING) {
        displayInfo(
          CONTACT_INFO_MAPPING[contactKey],
          crmContactInformation.contact[contactKey]
        );
      }
      document
        .getElementById("contact-info")
        .appendChild('<div class="fw-divider"></div>');
      for (var contactKey in WORK_INFO_MAPPING) {
        displayInfo(
          WORK_INFO_MAPPING[contactKey],
          crmContactInformation.contact[contactKey]
        );
      }
    },
