...
function cachedResponse() {
  ...
  client.iparams.get("freshservice_subdomain").then(function (iparam) {
    client.request
      .get(
        `https://${iparam.freshservice_subdomain}.freshservice.com/api/v2/agents`,
        options
      )
      .then(function (data) {
        try {
          ...
          notify(
            "error",
            "Error while attempting to show issue, kindly refresh the page "
          );
        }
      })
...
function renderTable(table) {
  ...
  document
    ...
    .insertAdjacentHTML(
      "beforeend",
      `${tableContainer}${tableHead}${tableBody}${tableContent}</tbody></table>`
    );
}
