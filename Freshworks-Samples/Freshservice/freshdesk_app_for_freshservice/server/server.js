...
exports = {
  ...
  onTicketUpdateCallback: function (args) {
    ...
    printLog(
      dataConstants.info,
      "FS tkt created from FD",
      regex.test(tktSubject)
    );
    if (regex.test(tktSubject)) {
      ...
      printLog(
        dataConstants.info,
        "Is status changed",
        JSON.stringify(args.data.ticket.changes.status)
      );
      if (args.data.ticket.changes.status) {
        ...
        printLog(
          dataConstants.info,
          "On status change in FS",
          args.iparams.fs_status_updated
        );
        if (
          args.iparams.fs_status_updated == dataConstants.add_private_note_in_fd
        ) {
          var fsGetTktStatusFieldURL =
            fsBaseURL + urlConstants.ticket_status_url;
          getRequestApi(
            fsGetTktStatusFieldURL,
            args,
            "Freshservice status details fetch"
          )
            .then((fsTktStatusData) => {
              var statusDetails = JSON.parse(fsTktStatusData.response)
                .ticket_fields[0];
            ...
            .catch((e) => {
              printLog(
                dataConstants.error,
                "Error in FS status details fetch",
                e
              );
            });
        } else {
          printLog(
            dataConstants.info,
            "Do nothing in Freshdesk on status change in Freshservice"
          );
        }
      }
    }
  ...
  onConversationCreateCallback: function (args) {
    printLog(
      ...
      args.iparams.fs_note_added
    );
    if (
      args.iparams.fs_note_added == dataConstants.add_private_note_in_fd &&
      ...
      getRequestApi(getFsTktURL, args, "Fetch FS Ticket details")
        .then((fsTktData) => {
          printLog(
            dataConstants.info,
            "Freshservice Ticket fetched successfully"
          );
          ...
          if (regex.test(tktSubject)) {
            var resultStr = tktSubject.match(regex)[1];
            var lastIndex = resultStr.lastIndexOf("-");
            var fdTktId = resultStr.substring(
              lastIndex + 1,
              resultStr.length - 1
            );
            var fdTktAddPvtNoteURL =
            ...
            postRequestAPI(
              ...
              .then((noteData) => {
                printLog(
                  dataConstants.info,
                  "Private Note created successfully in Freshdesk",
                  noteData
                );
              })
              .catch((e) => {
        ...
        .catch((e) => {
          printLog(
            dataConstants.error,
            "Error while fetching Freshservice Ticket",
            e
          );
        });
    } else {
      printLog(
        dataConstants.info,
        "Do nothing in Freshdesk when note is added in Freshservice"
      );
    }
  },
};
