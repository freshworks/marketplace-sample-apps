  ...
  return new Promise((resolve, reject) => {
    $request
      .get(url, {
        headers: {
          Authorization:
            product == dataConstants.freshdesk
              ? util.getFdAPIKey(args)
              : util.getFsAPIKey(args),
        },
      })
      .then(
        function (data) {
...
function createTicket(args, eventType) {
  ...
  var fdBaseURL =
    urlConstants.protocol +
    args.iparams.fd_subdomain +
    urlConstants.fd_domain_suffix;
  var fsBaseURL =
    urlConstants.protocol +
    args.iparams.fs_subdomain +
    urlConstants.fs_domain_suffix;
  ...
  if (
    args.data.ticket.responder_id != null &&
    args.data.ticket.responder_id != ""
  ) {
    for (var rule in ruleMap) {
      ...
      if (rule.indexOf("rule_") != -1) {
        ...
        if (
          args.data.ticket.status == ruleMap[rule].status &&
          ...
          getRequestApi(
            ...
            .then((agentData) => {
              ...
              printLog(
                dataConstants.info,
                "is_department_manadatory",
                args.iparams.fsMandatoryFields["is_department_manadatory"]
              );
              if (
                args.iparams.fsMandatoryFields["is_department_manadatory"]
                  .value == true
              ) {
                var fsGetRequesterURL =
                  fsBaseURL +
                  urlConstants.fs_requester_url +
                  agentDetails.contact.email;

                getRequestApi(
                  fsGetRequesterURL,
                  args,
                  "Requester Data Fetch",
                  dataConstants.freshservice
                )
                  .then((requesterData) => {
                    printLog(
                      dataConstants.info,
                      "Requester Data fetched successfully",
                      requesterData.response
                    );
                    var requesters = JSON.parse(
                      requesterData.response
                    ).requesters;
                  ...
                  .catch(function (e) {
                    printLog(
                      dataConstants.error,
                      "Error in fetching Freshservice requester",
                      e
                    );
                  });
              } else {
...
function createTicketInFS(
  ...
  postRequestAPI(fsAddTktURL, options, "Ticket Creation")
    .then((ticketData) => {
      ...
      setDataInDataStore(args.data.ticket.id, {
        fsTicketId: fsTicketDetails.id,
      });
      if (eventType == dataConstants.event_ticket_update) {
        if (args.iparams.ruleMap.add_private_note) {
          var fdTktGetConvoURL =
            fdBaseURL +
            urlConstants.ticket_url +
            args.data.ticket.id +
            urlConstants.conversations;
          getRequestApi(
            fdTktGetConvoURL,
            args,
            "Freshdesk Conversation fetch",
            dataConstants.freshdesk
          )
            .then((fdConvoData) => {
...
function getTicketPropertiesJson(
  args,
  requester_email,
  fsBaseURL,
  requestersDeptId
  ...
  printLog(
    dataConstants.info,
    "Freshservice Mandatory Tkt Fields" + JSON.stringify(fsMandatoryTktFlds)
  );
    ...
    if (key != "is_department_manadatory") {
      ...
      if (customFldTypesList.includes(currentKey.type)) {
        if (currentKey.type == "custom_number") {
          ...
          var formattedDate = moment(currentKey.value, ["YYYY-MM-DD"]).format(
            "YYYY-MM-DD"
          );
...
function setDataInDataStore(key, value) {
  $db.set(key, value).then(
    function (dbSuccessData) {
      printLog(
        dataConstants.info,
        "Data saved successfully in data store",
        JSON.stringify(dbSuccessData)
      );
    },
    function (dbError) {
      printLog(
        dataConstants.error,
        "Error in saving data in data store",
...
exports = {
    ...
    getDataFromDataStore(args.data.ticket.id)
      .then((dbResponse) => {
        if (dbResponse.status == "success") {
          printLog(
            dataConstants.info,
            "FS Ticket is already created for this FD Ticket",
            JSON.stringify(dbResponse.data)
          );
          if (args.data.ticket.changes.status) {
            ...
            printLog(
              dataConstants.info,
              "On FD Status Update",
              args.iparams.fd_status_updated
            );
            if (
              args.iparams.fd_status_updated ==
              dataConstants.add_private_note_in_fs
            ) {
              var fdGetTktStatusFieldURL =
                fdBaseURL + urlConstants.ticket_status_url;
              getRequestApi(
                fdGetTktStatusFieldURL,
                args,
                "Freshdesk status details fetch",
                dataConstants.freshdesk
              )
                .then((fdTktStatusData) => {
                  ...
                  var oldStatus =
                    statusChoices[args.data.ticket.changes.status[0]][0];
                  var newStatus =
                    statusChoices[args.data.ticket.changes.status[1]][0];
                ...
                .catch(function (e) {
                  printLog(
                    dataConstants.error,
                    "Error in FD status details fetch",
                    JSON.stringify(e)
                  );
                });
            } else {
              printLog(
                dataConstants.info,
                "Do nothing in Freshservice on status change in Freshdesk"
              );
            }
          }
        } else {
      ...
      .catch((e) => {
        printLog(
          dataConstants.error,
          "Error in fetching data from data store",
          JSON.stringify(e)
        );
      });
  },
  ...
  onConversationCreateCallback: function (args) {
    printLog(
      dataConstants.info,
      "On FD Note Create",
      args.iparams.fd_note_added
    );
    if (
      args.iparams.fd_note_added == dataConstants.add_private_note_in_fs &&
          ...
          if (dbResponse.status == "success") {
            ...
            postRequestAPI(fsTktAddCovoURL, fsAddNoteOptions, "Note Creation")
              ...
              .catch(function (e) {
                printLog(
                  dataConstants.error,
                  "Error in conversation Creation",
                  JSON.stringify(e)
                );
              });
          } else {
            printLog(
              dataConstants.error,
              "This ticket is not present in FS",
      ...
      printLog(
        dataConstants.info,
        "Do nothing in Freshservice on note creation in Freshdesk"
      );
    }
  },
};
