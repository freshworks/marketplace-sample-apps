...
function timerValidation(event) {
  client.data.get("ticket").then(
    (ticketData) => ticketDataRequest(ticketData, event),
    (e) => handleErrors(e, event)
  );
}

function ticketDataRequest(ticketData, event) {
  client.iparams.get("freshdesk_domain").then(
    function (iparam) {
...
function timerDataRequestHandler(data, event) {
  if (data.status === 200) {
    const runningTimer = JSON.parse(data.response).some(
      (timeEntry) => timeEntry.timer_running === true
    );
    const failText =
      "Timer(s) running. Stop the timer(s) before proceeding to close the ticket.";

    runningTimer ? event.helper.fail(failText) : event.helper.done();
  } else {
