exports = {
  ...
  getBookmarkedTickets: async function (request) {
    getAgentTicketsFromDb(request.agentId).then(
      ...
      function (errorMessage) {
        console.error(
          "Error occurred while fetching tickets from DB " + errorMessage
        );
        renderData(null, []);
      }
    );
  ...
  removeAgentsTicket: async function (request) {
    ...
    removeAgentTicket(request.agentId, request.ticketId).then(
      function (data) {
        renderData(null, data);
      },
      function (error) {
...
async function saveTicketForAgent(request) {
  try {
    ...
    if (existingTickets === null || existingTickets.savedTickets === null) {
      ...
      let saveTicketResponse = await saveTicketInDb(
        request.agentId,
        request.ticketId,
        request.ticketSubject
      );
      console.log(
        "Save ticket response for first ticket " +
          JSON.stringify(saveTicketResponse)
      );
    ...
    console.log(
      "There are few existing tickets for agent: " +
        existingTickets.savedTickets
    );
    let filteredTicketByRequestedId = existingTickets.savedTickets.filter(
    ...
    console.log(
      "Save ticket response for adding ticket " +
        JSON.stringify(saveTicketResponse)
    );
...
async function removeAgentTicket(agentId, ticketId) {
  try {
    ...
    console.log(
      "There are few existing tickets for agent with ID " +
        agentId +
        ": " +
        JSON.stringify(existingTickets.savedTickets)
    );
    let filteredTicketByRequestedId = existingTickets.savedTickets.filter(
    ...
    if (filteredTicketByRequestedId.length == 0) {
      console.info(
        "No ticket found with ticket ID " +
          ticketId +
          " for agent ID " +
          agentId
      );
    ...
    let remainingTickets = existingTickets.savedTickets.filter(
      (ticket) => ticket.ticketId !== ticketId
    );

    return $db
      .update("agentId: " + agentId, "set", { savedTickets: remainingTickets })
      .then(
        function (response) {
          console.info(
            "Removed the ticket " + ticketId + " from agent with id " + agentId
          );
          return response;
        },
        function (error) {
...
function saveTicketInDb(agentId, ticketId, ticketSubject) {
  console.log(
    "Saving ticket in DB for agent ID " + agentId + " and ticket ID " + ticketId
  );
  if (
    agentId === null ||
    agentId === undefined ||
  ...
  return $db
    .update("agentId: " + agentId, "append", {
      savedTickets: [{ ticketId: ticketId, ticketSubject: ticketSubject }],
    })
    .then(
      function (response) {
