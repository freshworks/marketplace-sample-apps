...
function saveNote() {
  ...
  if (note === "") {
    showNotification(
      "warning",
      "Note is empty. Type something to save the note."
    );
    return;
  }
  client.data.get("conversation").then(
    function (conversationData) {
      const encodedConversationId = encode(
        conversationData.conversation.conversation_id
      );
...
function getNote() {
  ...
  client.data.get("conversation").then(
    function (conversationData) {
      client.request
        .invoke("getNote", {
          conversation_id: conversationData.conversation.conversation_id,
        })
        .then(
          function (noteData) {
    ...
    .then(function (_client) {
      ...
      client.events.on("app.activated", function () {
        ...
        document
          ...
          .removeEventListener("click", saveNote);
        document
          .getElementById("btnSaveNote")
          .addEventListener("click", saveNote);
        document
          .getElementById("btnGetNote")
          .removeEventListener("click", getNote);
        document
          .getElementById("btnGetNote")
          .addEventListener("click", getNote);
      });
    })
